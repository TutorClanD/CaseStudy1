...
for iter=1:max_iter
    ...
    for i = 1:size(train, 1)
        current_vector = train(i, 1:784); 

        [idx, dist] = assign_vector_to_centroid(current_vector, centroids);
...
figure;
plot(1:max_iter, cost_iteration, 'b-', 'LineWidth', 2);
xlabel('Number of Iterations');
ylabel('K-means Cost (Sum of Squared Distances)');
title('K-means Cost vs. Iterations');
grid on;
...
for i = 1:K
    cluster_vectors = data(data(:, 785) == i, 1:784);
    
    if ~isempty(cluster_vectors)
        new_centroids(i, :) = mean(cluster_vectors, 1);
    else
        new_centroids(i, :) = zeros(1, 784); 
    end
end

end

%% Part 2: Assign labels to centroids (Automatic)
k = size(centroids, 1);
centroid_labels = zeros(k, 1);

for i = 1:k
    % Find all training images assigned to centroid i
    assigned_idx = find(train(:, 785) == i);

    if isempty(assigned_idx)
        warning('Centroid %d has no assigned training images.', i);
        centroid_labels(i) = -1;
        continue;
    end

    assigned_labels = trainsetlabels(assigned_idx);

    centroid_labels(i) = mode(assigned_labels);
end

disp('Centroid labels (automatic):');
disp(centroid_labels');




%% Part 2: Assign labels to centroids (Visual)
%k = size(centroids, 1);
%centroid_labels = zeros(k, 1);

% Re-display the centroid montage (Figure 2) so you can read off digits,
% or view them individually here:
%figure;
%for i = 1:k
%    subplot(ceil(k/5), 5, i);
...
% Enter the label for each centroid by inspection
%for i = 1:k
%    prompt = sprintf('Enter digit label for Centroid %d: ', i);
%    centroid_labels(i) = input(prompt);
%end

%disp('Centroid labels (manual):');
%disp(centroid_labels');
